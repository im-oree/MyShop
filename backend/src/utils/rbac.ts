import { EmployeePermissions, EmployeeRoleTemplate, AccessLevel, User } from '../types/index.js'
...
export function normalizePermissions(input?: Partial<EmployeePermissions> | null): EmployeePermissions {
...
export function resolveTemplatePermissions(
  ...
  customPermissions?: Partial<EmployeePermissions> | null,
): EmployeePermissions {
  if (!template || template === 'custom') {
    return normalizePermissions(customPermissions)
  }

  return EMPLOYEE_ROLE_TEMPLATES[template] || normalizePermissions(customPermissions)
}
