...
function SellerOrderDetailPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        setLoading(true)
        const res = await orderService.getSellerOrderById(id)
  ...
  const handleAdvance = async () => {
    ...
    try {
      ...
      const updated = await orderService.getSellerOrderById(order.id)
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-4 flex items-center gap-3">
        ...
        {user?.role === 'seller' && normalizeOrderStage(order.status) !== 'completed' && (
