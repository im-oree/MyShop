...
function SellerProductsPage() {
  ...
  const canManageProducts = rbac.hasAccess(perms.products, 'write')
  ...
  useEffect(() => {
    ...
    const loadSellerProducts = async () => {
      setLoading(true)
      try {
        const response: any = await productService.getAll(1, 1000)
        setProducts(response.items || response.products || [])
      } catch (error) {
    ...
    // read stock filter from URL (e.g. /admin/store/products?stock=out)
