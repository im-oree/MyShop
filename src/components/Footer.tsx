...
import { Home, Shield, ShoppingBag, UserRound, LayoutDashboard } from 'lucide-react'
...
function Footer() {
  ...
  const { user, viewMode, isAuthenticated } = useAuthStore()
  ...
  const getMobileLinks = () => {
    if (isStaffMode) {
      return [
        { to: '/admin/store', label: 'Dashboard', icon: Shield },
        { to: '/admin/store/products', label: 'Products', icon: ShoppingBag },
        { to: '/admin/store/orders', label: 'Orders', icon: LayoutDashboard },
      ...
      <footer className="bg-primary text-white mt-auto hidden md:block">
        <div className="container mx-auto px-4 py-10 sm:py-12">
              ...
              <div className="flex items-center gap-3 mt-5">
                {['twitter', 'github', 'instagram'].map((social) => (
                  <a
                    ...
                    href="#"
                    aria-label={social}
                    className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20 active:scale-95 transition-base"
                  >
                ...
                <ul className="space-y-2.5">
                  {section.links.map((link) => (
                    <li key={link.label}>
                      <a
                        href={link.href}
                        className="text-sm text-gray-300 hover:text-white hover:translate-x-0.5 inline-block transition-base"
                      >
                        {link.label}
                      </a>
                    </li>
          ...
          <div className="border-t border-white/10 mt-10 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <p className="text-xs text-gray-500">Built with ❤️ for great shopping</p>
          </div>
        </div>
      </footer>

      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-white/95 backdrop-blur-md">
        <div className={`grid gap-1 px-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] pt-2 ${
          isStaffMode ? 'grid-cols-4' : 'grid-cols-3'
        }`}>
          {mobileLinks.map((link) => {
            const active = location.pathname === link.to
            const Icon = link.icon
            const isOrders = link.label === 'Orders'
            const showOrdersBadge = isOrders && incompleteOrders > 0

            return (
              <Link
                key={link.to}
                to={link.to}
                className={`relative flex min-w-0 flex-col items-center justify-center gap-1 rounded-2xl px-1.5 py-2 text-[10px] sm:text-[11px] font-medium leading-none transition-all ${
                  active ? 'text-primary bg-primary/5' : 'text-muted-text hover:text-text'
                }`}
              >
